    ...
    <header className="fixed top-0 left-0 right-0 z-50 px-4 py-6 pointer-events-none">
      <nav ref={navRef} className="mx-auto flex max-w-5xl items-center justify-between px-3 py-2 rounded-full bg-surface/60 backdrop-blur-xl border border-glass-border shadow-lg pointer-events-auto">
        ...
        <div className="flex gap-4 md:gap-6 items-center pr-2 pl-2">
          ...
          {isAdmin && (
            <Link href={`/${lang}/admin/dashboard`} className="nav-item group flex items-center gap-2 px-4 py-2 rounded-full bg-brand/10 border border-brand/20 hover:border-brand/40 hover:bg-brand/20 transition-all duration-300 shadow-[0_0_10px_rgba(var(--brand-color),0.1)]">
              <svg className="w-4 h-4 text-brand-light shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
              </svg>
              <span className="text-brand-light font-bold text-sm hidden md:block">{lang === 'ar' ? 'الداشبورد' : 'Dashboard'}</span>
            </Link>
          )}

          {/* Portfolio Button */}
          <Link href="https://wb6ya.com" target="_blank" className="nav-item group flex items-center gap-2 px-3 md:px-4 py-2 rounded-full border border-transparent hover:border-glass-border hover:bg-white/5 transition-all duration-300">
            <span className="text-muted-foreground group-hover:text-foreground text-sm font-medium transition-colors hidden md:block">{dict?.personalSite || 'My Portfolio'}</span>
            <svg className="w-4 h-4 md:w-3.5 md:h-3.5 text-muted-foreground group-hover:text-brand-light transition-colors group-hover:translate-x-0.5 group-hover:-translate-y-0.5 duration-300 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
