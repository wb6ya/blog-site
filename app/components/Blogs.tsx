...
function Blogs({ blogs, dict, heroDict, lang, currentPage, totalPages, currentSearch = "", currentTag = "", systemTags = [], popularBlogs = [] }: BlogsProps) {
  ...
  return (
    <div ref={container} className="relative z-10 flex flex-col gap-12">
      ...
      {/* Trending Section */}
      {showHero && popularBlogs.length > 0 && (
        <div className="my-12">
          <div className="flex items-center gap-3 mb-8">
            <span className="flex items-center justify-center w-10 h-10 rounded-xl bg-orange-500/10 text-orange-500 border border-orange-500/20 shadow-[0_0_15px_rgba(249,115,22,0.2)]">
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M17.5 12.5c0 2.5-2.5 4.5-5.5 4.5s-5.5-2-5.5-4.5c0-1.5 1-3 2-4 .5-.5 1-1.5 1-2.5 0-1.5-1-2.5-1-2.5s2.5 1 4.5 3c2 2 2.5 4 4.5 4 1 0 2-1 2-2 0 0 .5 1.5.5 2.5z" />
              </svg>
            </span>
            <h3 className="text-2xl font-bold text-foreground tracking-wide">
              {lang === 'ar' ? 'الأكثر شعبية' : 'Trending Articles'}
            </h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {popularBlogs.map((blog) => (
              <BlogCard key={`trending-${blog._id}`} blog={blog} dict={dict} lang={lang} />
            ))}
          </div>
        </div>
      )}
