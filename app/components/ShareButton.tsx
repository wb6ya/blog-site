...
export default function ShareButton({ lang, title }: ShareButtonProps) {
  ...
  return (
    <button 
      onClick={handleShare}
      className={`px-6 py-2.5 rounded-full backdrop-blur-md border transition-all text-sm font-medium flex items-center gap-2 group cursor-pointer ${
        copied 
          ? 'bg-green-500/20 border-green-500/50 text-green-400' 
          : 'bg-surface/50 border-glass-border hover:bg-white/5 hover:border-brand/40 text-foreground'
      }`}
    >
      {copied ? (
